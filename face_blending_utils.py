...
def get_face_region_with_padding(landmarks: np.ndarray, img_shape: Tuple[int, int],
    ...
    Returns:
        (xmin, ymin, xmax, ymax) with padding
    """
    h, w = img_shape[:2]
...
def blend_faces(original_img: np.ndarray, generated_face: np.ndarray,
    ...
    Args:
        ...
        generated_face: Generated face region (same size as original)
        landmarks: Facial landmarks
        feather_amount: Amount of feathering at edges
...
def align_landmarks_to_reference(landmarks: np.ndarray, 
    ...
    Args:
        landmarks: Current landmarks to align
        reference_landmarks: Reference landmarks to align to
